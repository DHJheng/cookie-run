...
void menu_process(ALLEGRO_EVENT event){
    if( event.type == ALLEGRO_EVENT_MOUSE_BUTTON_DOWN ) {   // if( event.type == ALLEGRO_EVENT_MOUSE_BUTTON_UP ) {
        if( mouse_in_range(event.mouse.x, event.mouse.y, WIDTH/2-150, 510, WIDTH/2+150, 550) ) {
            judge_next_window = true;
            menu_button_type = 1;
        } else if( mouse_in_range(event.mouse.x, event.mouse.y, WIDTH/2-150, 410, WIDTH/2+150, 450) ) {
            judge_next_window = true;
            menu_button_type = 2;
        } else if( mouse_in_range(event.mouse.x, event.mouse.y, WIDTH/2-150, 310, WIDTH/2+150, 350) ) {
            judge_next_window = true;
            menu_button_type = 3;
        }
    }
...
void menu_draw(){
    al_clear_to_color(al_map_rgb(100,50,0));   // al_clear_to_color(al_map_rgb(100,100,100));
